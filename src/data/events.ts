import watoc2028 from '@/assets/events/watoc2028.png'
import aroma2018 from '@/assets/events/aromaticity2018group.png'
import aroma2025 from '@/assets/events/aroma2025.png'

export const events = [
    {
        name: 'WATOC 2028',
        href: 'https://www.watoc2028.org/',
        description: [
            'The World Association of Theoretical and Computational Chemists will host its triennial congress in Merida, Mexico (January 9 — 14, 2028).',
            'Organized by Gabriel Merino, the event promotes scientific exchange through sessions like Young WATOC and the presentation of the Schrödinger and Dirac medals.',
        ],
        img: watoc2028,
    },
    {
        name: 'Aromaticity 2025',
        href: '#',
        description: [
            'International congress held in Merida, Mexico (January 27–30, 2025) at the Mayan World Museum.',
            'Organizers Gabriel Merino, Miquel Solà, and Henrik Ottosson gathered global experts to establish a standardized IUPAC definition of aromaticity, featuring sessions at Sala Mayamax and excursions to Uxmal and Chichen Itza.',
        ],
        img: aroma2025,
    },
    {
        name: 'Aromaticity 2018',
        description: [
            'International congress held in the Riviera Maya (Nov 28 – Dec 1, 2018) led by Gabriel Merino.',
            'It gathered experts to discuss and update the IUPAC definition of aromaticity, focusing on molecular structure and reactivity.',
        ],
        href: '#',
        img: aroma2018,
    },

]